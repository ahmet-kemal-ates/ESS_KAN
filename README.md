...
## Data Placement
Put all RW files in `data/`:
- `trainRW9.mat`, `trainRW10.mat`, ...
- `testRW9.mat`, `testRW10.mat`, ...

Important:
- current pipeline automatically reads all `trainRW*.mat` and `testRW*.mat` from `data/`
- values in `configs/base.yaml` (`train_mat`, `test_mat`) are used to resolve the data directory
